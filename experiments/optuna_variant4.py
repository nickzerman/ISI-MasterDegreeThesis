...
def run(n_trials=50, fixed_unified=None, data_file=None):
    ...
    def objective(trial, fixed_unified):
        ...
        try:
            return train_unified(model, data, config, device, trial=trial)[0]
        finally:
            del model
            gc.collect()
    ...
    study = optuna.create_study(
        direction='minimize', sampler=sampler, pruner=pruner, study_name='v4_unified',
