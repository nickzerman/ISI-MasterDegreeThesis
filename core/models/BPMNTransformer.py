...
class BPMNTransformer(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is None:
            ...
            # Prendo solo l'ultima colonna (Previsione)
            targets_last = targets # (B, num_bits)

            # Sigmoide come detto dal prof.
            pred = torch.sigmoid(logits_last)

            B = (1 - targets_last).sum(dim=1, keepdim=True) / self.num_bits

            eps= 1e-7
            loss = -( (1-B) * (1 - targets_last) * torch.log(1 - pred + eps) + B * targets_last * torch.log(pred + eps) ).mean()
